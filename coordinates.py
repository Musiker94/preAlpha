...
closer = pd.read_csv('DATA/Evolut/closer/particles.csv', ' ')
coordinates = np.zeros([len(closer), 8])
for count in range(0, len(closer)):
    elem = closer[closer.index == count].get_values()[0]
    coordinates[count, 0:3] = AstroClass.kepler2descart(elem[0:6])[0]
    coordinates[count, 3:6] = AstroClass.kepler2descart(elem[0:6])[1]
    coordinates[count, 6] = np.sqrt(np.dot(coordinates[count][0:3], coordinates[count][0:3]))
    coordinates[count, 7] = elem[6]
    print('Count = %i' % count)

np.savetxt('DATA/Evolut/closer/coordinates.csv', coordinates, header='X Y Z velX velY velZ r year', comments='')
